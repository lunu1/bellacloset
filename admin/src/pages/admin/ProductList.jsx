...
import { useGetProductsQuery, useDeleteProductMutation } from "../../features/adminProducts/adminProductsApi";
import useDebouncedValue from "../../hooks/useDebouncedValue";
import { getDisplayFields } from "../../utils/productDisplay";

export default function ProductsList() {
  // UI state (keep in component or move to a uiSlice if you want persistence across routes)
  ...
  const queryArg = useMemo(
    () => ({ page, limit, search: debouncedSearch || undefined, sortBy, sortOrder }),
    [page, limit, debouncedSearch, sortBy, sortOrder]
  );

  const { data, isLoading, isError, error, refetch, isFetching } = useGetProductsQuery(queryArg);
  const [deleteProduct, { isLoading: isDeleting }] = useDeleteProductMutation();

  // Normalize response (array or object-with-meta)
  const items = useMemo(() => (Array.isArray(data) ? data : data?.items || []), [data]);
  const total = useMemo(() => {
    if (Array.isArray(data)) return data.length; // no server paging
    return typeof data?.total === "number" ? data.total : items.length;
  }, [data, items]);

  const totalPages = Math.max(1, Math.ceil(total / limit));
  const showClientSlice = Array.isArray(data); // if server doesn't paginate, slice on client
  ...
  return (
    <div className="p-6">
      <div className="flex items-center gap-3 flex-wrap mb-4">
        ...
        />
        <select value={limit} onChange={(e) => { setPage(1); setLimit(Number(e.target.value)); }} className="border px-2 py-2 rounded">
        ...
        <button onClick={() => refetch()} className="border px-3 py-2 rounded">Refresh</button>
      ...
      {isLoading ? (
        ...
        <div className="text-red-600">{error?.data?.message || error?.error || "Failed to load"}</div>
      ) : (
          ...
          <table className="min-w-full text-sm">
            ...
            <tbody>
              {pagedItems.map(({ product, variants = [] }) => {
                const { image, price, compareAtPrice, stock } = getDisplayFields(product, variants);
                return (
                  <tr key={product._id} className="border-t">
                    ...
                    <Td>{product.brand || <span className="text-gray-400">—</span>}</Td>
