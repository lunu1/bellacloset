...
const ProductUpdateForm = ({ initialProduct, initialVariants = [], onSubmit, submitting }) => {
  ...
  const handleDefaultImageUpload = async (e) => {
    ...
    try {
      const res = await axios.post(`${import.meta.env.VITE_API_URL || "http://localhost:4000/api"}/upload/images`, formData);
  ...
  const handleSubmit = async (e) => {
    ...
    const productPatch = {
      ...
      tags: tagsArray,
      options: hasVariants ? ["Color", "Size"].filter((o) => product.options.includes(o)) : [], // keep your options logic
    ...
    const variantsPayload = hasVariants ? variants.map((v) => ({
  ...
  return (
    <form onSubmit={handleSubmit} className="space-y-4">
      ...
      <select name="category" className="border p-2 w-full" value={product.category} onChange={handleChange}>
      ...
      {!hasVariants && (
        <>
          ...
          <input type="number" placeholder="Comparison Price" className="border p-2 w-full"
            value={compareAtPrice} onChange={(e) => setCompareAtPrice(e.target.value)} />
          <input type="number" placeholder="Stock" className="border p-2 w-full"
            value={defaultStock} onChange={(e) => setDefaultStock(e.target.value)} />
          ...
          <div className="border border-dashed border-gray-300 p-4 rounded-md">
            ...
            <div className="flex items-center gap-4">
              ...
              {uploadingDefault && <span className="text-sm text-gray-500">Uploading…</span>}
            </div>
            <div className="flex gap-2 mt-4 flex-wrap">
              {defaultImages.map((img, i) => (
                <div key={i} className="relative group">
                  <img src={img} className="w-16 h-16 object-cover rounded" alt="preview" />
                  <button
                    type="button"
      ...
      <button
        type="submit"
        className={`px-6 py-2 text-white rounded ${submitting ? "bg-gray-400 cursor-not-allowed" : "bg-blue-600"}`}
        disabled={submitting}
      >
