...
export default function OrderItems({ order }) {
  const { format } = useCurrency(); // ✅ use global currency
  ...
  const lines = (order?.products || []).map((item) => {
    ...
    const image =
      item.image || v?.images?.[0] || p?.images?.[0] || "/placeholder.jpg";
          ...
          {lines.map((line, i) => (
            ...
            >
              ...
              <div className="text-right">
                <div className="font-medium">{format(line.unit)}</div>
                <div className="text-xs text-gray-500">each</div>
                <div className="text-xs text-gray-500 mt-1">
                  Line: {format(line.lineTotal)}
                </div>
              </div>
            </div>
          ))}
